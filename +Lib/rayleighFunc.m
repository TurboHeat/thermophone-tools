function [Mag_Pff_1, Mag_Prms_ff_1, Mag_Prms_nf_ff_1, Ph_Pff_1, ...
    ...
    rayleighFunc(Ny, Nz, Dimensions, MDM, kay1, kay2, vM, cumLo, maxO, Omega)
...
Factor2 = (Dimensions(1)) .* (Dimensions(2)) .* (1i .* Omega .* MDM(end, 2) / (2 * pi * numel(r)));

Ro1 = (Dimensions(1)) .* (Dimensions(2)) .* Omega ./ (4 * pi * sqrt(MDM(1, 3) * MDM(1, 7) / (MDM(1, 8) * MDM(1, 2))));
Ro2 = (Dimensions(1)) .* (Dimensions(2)) .* Omega ./ (4 * pi * sqrt(MDM(end, 3) * MDM(end, 7) / (MDM(end, 8) * MDM(end, 2))));

% Preallocation:
[Mag_Pff_1, Mag_Pff_2, Ph_Pff_1, Ph_Pff_2] = deal(zeros(1, maxO));

for k = 1:maxO
    Mag_Pff_1(k) = abs(Factor1(k)*V_front(k)*sum(sum((1./r) .* exp(-1i * kay1(k) .* r))));
    Mag_Pff_2(k) = abs(Factor2(k)*V_back(k)*sum(sum((1./r) .* exp(-1i * kay2(k) .* r))));
    %Phase
