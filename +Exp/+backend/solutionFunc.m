function [results] = solutionFunc(layers, simConfigMat, calibOpts, expfilenames)
...
% Omega = 2 * pi * datapoints.("Current Frequency")(1); % If datapoints was still a table

% Updating the generation with the experimental data
[Sth] = Exp.backend.inputpowercalc(Va, Vb, Vc, datapoints);
...
[BCI, Hamat, w1_c, ~, Smat, Hbmat, SCALE] = Lib.systemSolver(Omegamp, MDMmp, nLayers, cumLomp);
...
[Mag_Pff_1, Mag_Prms_ff_1, Mag_Prms_nf_ff_1, Ph_Pff_1, ...
    ~, ~, ~, ~] = ...
    Lib.rayleighFunc(Ny, Nz, simConfigMat, MDM, kay1, kay2, vM, cumLo, 1, Omega);

%% ---------------------------------------------------------%%
% Calibration of instruments
[Tpile, P0, T0] = calibOpts.applyToData(datapoints(:, 2), datapoints(1, 5), datapoints(1, 3));

%% ---------------------------------------------------------%%
% Calculation of experimentally measured rms sound pressure
[Mag_Prms_EXP, ~] = Exp.backend.dft(MIC, datapoints(1, 13:16));
...
% Calculation of experimentally measured rms sound pressure
[Mag_ACCrms_EXP, acc_freq] = Exp.backend.dft(ACC, datapoints(1, 13:16));

%Mag_Prms_EXP=Mag_Prms_ff_1; %for testing purposes

%% ---------------------------------------------------------%%
% Inverse calculation of acoustic propagation and determination of boundary
% heat-flux, pressure and temperature oscillations. Velocity measure is at the
% L-th location.
[~, V_front, Q_front, ~] = ...
...
[W_front, V_rms, Jth, Jv, Ratio_pred] = Exp.backend.vibroFunc(P0, T0, simConfigMat, MDM, Mag_ACCrms_EXP, 2*pi*acc_freq, V_front);
...
results = ThermophoneModels.ExperimentResults("ETA1", ETA1, "PRES1", PRES1, ...
