...
[T_mean,q_mean]=Mean_temp(MDM,N_layers,cumLo,x,cc1);
...
p=zeros(maxO,Nq);q=p;T=p;v=p;T_m=p;
pM=zeros(maxO,N_layers+1);TM=pM;vM=pM;qM=pM;T_Mm=pM;
...
for j=1:maxO
    ...
    [BCI,Hamat,w1_c,w2_c]=T_system_solver(Omega(j),invHa_f,invHb_f,Ha_f,Hb_f,w1_f,w2_f,S_f,MDM,N_layers,cumLo,aaN,ccN,bb1,dd1,Dimensions);
    
    for que=1:Nq % x-position loop
        
        [field,interval]=Interrogation(Hamat,Hb_f,S_f,cumLo,Posx(que),N_layers,BCI,T_mean,MDM,Omega(j),w1_c,w2_c);
        p(j,que)=field(1,:);v(j,que)=field(2,:);q(j,que)=field(3,:);...
            T(j,que)=field(4,:);T_m(j,que)=field(5,:);
    ...
    for que=2:N_layers % x-position loop
        ...
        [field,interval]=Interrogation(Hamat,Hb_f,S_f,cumLo,que,N_layers,BCI,T_mean,MDM,Omega(j),w1_c,w2_c);
        pM(j,que)=field(1,:);vM(j,que)=field(2,:);qM(j,que)=field(3,:);
        TM(j,que)=field(4,:);T_Mm(j,que)=field(5,:);
        
    end
    
    %% At the maximum velocity position in the first and last layers (thermal thickness)
    [field,~]=Interrogation(Hamat,Hb_f,S_f,cumLo,cumLo(1)-(2*(MDM(1,9)/(2*MDM(1,7)*Omega(j)*MDM(1,2)))^(1/2)),N_layers,BCI,T_mean,MDM,Omega(j),w1_c,w2_c);
    pM(j,1)=field(1,:);vM(j,1)=field(2,:);qM(j,1)=field(3,:);
    TM(j,1)=field(4,:);T_Mm(j,1)=field(5,:);
    
    [field,~]=Interrogation(Hamat,Hb_f,S_f,cumLo,cumLo(end)+(2*(MDM(1,9)/(2*MDM(1,7)*Omega(j)*MDM(1,2)))^(1/2)),N_layers,BCI,T_mean,MDM,Omega(j),w1_c,w2_c);
    pM(j,N_layers+1)=field(1,:);vM(j,N_layers+1)=field(2,:);
    qM(j,N_layers+1)=field(3,:);TM(j,N_layers+1)=field(4,:);
...
[eta_TP_1_A,eta_TP_2_A,eta_TP_1,...
    ...
    eta_Tot_1,eta_Tot_2]=...
    Efficiency_calc(pM,TM,qM,vM,Dimensions(12),N_layers,P_in,MDM);
