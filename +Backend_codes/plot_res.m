...
if numel(Omega) ~= 1
  ...
  f = figure(13);
  hold all;
  plot(Omega/(2 * pi), PRES1(:, 2));
  plot(Omega/(2 * pi), PRES2(:, 2));
  ...
  f = figure(15);
  hold all;
  plot(Omega/(2 * pi), PRES1(:, 4));
  plot(Omega/(2 * pi), PRES2(:, 4));
  title('Phase field');
  xlabel('$f[Hz]$', 'fontsize', 14, 'interpreter', 'latex')
  ...
  plot(Omega/(2 * pi), ETA1(:, 1));
  plot(Omega/(2 * pi), ETA2(:, 1));
  plot(Omega/(2 * pi), ETA1(:, 4));
  plot(Omega/(2 * pi), ETA2(:, 4));
  plot(Omega/(2 * pi), ETA1(:, 5));
  plot(Omega/(2 * pi), ETA2(:, 5));
  plot(Omega/(2 * pi), ETA1(:, 3));
  plot(Omega/(2 * pi), ETA2(:, 3));
