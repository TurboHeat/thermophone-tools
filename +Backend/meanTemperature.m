...
control_h=false; %is h defined by the user? or do you want it overwritten
...
%% Black-body radiation (we assume epsilon=1)
sigma=5.670373*10^-8; %constant
...
if control_h==false %if h is defined by natural convection and is dictated by the code.
    ...
    T_est=double(vpasolve(sigma*(Temp^4-MDM(1,14)^4)+PB*((Temp-MDM(1,14))^(5/4))-G/2));    
    ...
    p=[1 0 0 (h1+h2)/(2*sigma) (((h1+h2)/(2*sigma)*MDM(1,14))-G/(2*sigma)-(MDM(1,14)^4)) ];
    temp=(roots(p));
...
q1=(MDM(1,12)+MDM(2,10))-sigma*(T_est^4-MDM(1,14)^4);
q2=(MDM(end,10)+MDM(end-1,12))-sigma*(T_est^4-MDM(1,14)^4);
